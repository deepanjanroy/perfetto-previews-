	...
	***************************************************************************** */
	...
	class MultiSelect {
	    view({ attrs }) {
	        ...
	        return mithril('label.multiselect', {
	            class: attrs.enabled ? '' : 'disabled',
	            for: `multiselect-${toId(attrs.label)}`,
	        }, attrs.label, mithril('div', helpers), mithril('input', {
	...
	class Numeric {
	    view({ attrs }) {
	        ...
	        }, p.label)), mithril('input[type=number][min=1]', {
	            id: `range-${toId(attrs.label)}`,
	            placeholder: attrs.placeholder,
	            value: attrs.value,
	            disabled: !attrs.enabled,
	            onchange: mithril.withAttr('value', s => attrs.onchange(toNumber(s))),
	        })), mithril('small', attrs.sublabel));
	    }
	}
	function onAdd(name) {
	    return (optionsToAdd) => {
	        globals.globals.dispatch(actions.Actions.addConfigControl({ name, optionsToAdd }));
	    };
	}
	function onSubtract(name) {
	    return (optionsToRemove) => {
	        globals.globals.dispatch(actions.Actions.removeConfigControl({ name, optionsToRemove }));
	    };
	}
	function onChange(name) {
	    return (value) => {
	        globals.globals.dispatch(actions.Actions.setConfigControl({ name, value }));
	    };
	}
	function isFalsy(x) {
	...
	exports.RecordPage = pages.createPage({
	    view() {
	        ...
	        return mithril('.record-page', { class: state.displayConfigAsPbtxt ? 'three' : 'two' }, mithril('.config.text-column', `To collect a Perfetto trace, configure the options below then
          use the command on the right to capture the trace.`, mithril('.heading', mithril(Numeric, {
	            enabled: true,
	            label: 'Duration',
	            sublabel: 's',
	            placeholder: '',
	            ...
	            enabled: true,
	            onchange: onChange('writeIntoFile'),
	        }), mithril('.control-group', mithril(Numeric, {
	            ...
	            help: '',
	            onchange: onChange('fileWritePeriodMs'),
	            presets: [
	            ...
	            onchange: onChange('bufferSizeMb'),
	            presets: [
	                { label: '1mb', value: 1 },
	                { label: '10mb', value: 10 },
	                { label: '20mb', value: 20 },
	            ]
	        }), mithril('.heading', mithril(Toggle, {
	            label: 'Record process/thread associations',
	            help: PROCESS_METADATA_HELP,
	            value: state.processMetadata,
	            enabled: true,
	            onchange: onChange('processMetadata'),
	        })), 
	        ...
	        mithril('.heading', mithril(Toggle, {
	        ...
	        }, 'Advanced ftrace configuration'), mithril(Numeric, {
	            enabled: state.ftrace,
	            ...
	            onchange: onChange('ftraceDrainPeriodMs'),
	            presets: [
	            ...
	            onchange: onChange('ftraceBufferSizeKb'),
	            presets: [
	            ...
	            onchange: onChange('vmstatPeriodMs'),
	            presets: COUNTER_PRESETS,
	        }), mithril(MultiSelect, {
	            label: 'Vmstat Counters',
	            enabled: state.sysStats && isTruthy(state.vmstatPeriodMs),
	            selected: state.vmstatCounters,
	            options: Object.keys(protos_1.VmstatCounters)
	                .filter(c => c !== 'VMSTAT_UNSPECIFIED'),
	            onadd: onAdd('vmstatCounters'),
	            onsubtract: onSubtract('vmstatCounters'),
	        })), mithril('hr'), mithril(Toggle, {
	            label: 'Display config as pbtxt',
	            value: state.displayConfigAsPbtxt,
	            enabled: true,
	            help: '',
	            onchange: onChange('displayConfigAsPbtxt'),
	        })), data ?
	            [
	                mithril('.command.text-column', `To collect a ${state.durationSeconds}
                second Perfetto trace from an Android phone run this command:`, mithril(CodeSample, { text: data.commandline }), 'Then click "Open trace file" in the menu to the left and select', ' "/tmp/trace".'),
	                state.displayConfigAsPbtxt ?
	                    mithril('.pbtxt.text-column', `A Perfetto config controls what and how much information is
                  collected. It is encoded as a `, mithril('a', { href: CONFIG_PROTO_URL }, 'proto'), '.', mithril(CodeSample, { text: data.pbtxt, hardWhitespace: true })) : null,
	            ] : null);
	    }
	});
	...
	class TrackGroupPanel extends panel.Panel {
	    ...
	    get summaryTrackState() {
	        return logging.assertExists(globals.globals.state.tracks[this.trackGroupState.summaryTrackId]);
	    }
	    view({ attrs }) {
	        const collapsed = this.trackGroupState.collapsed;
	        return mithril(`.track-group-panel[collapsed=${collapsed}]`, mithril('.shell', mithril('h1', `${this.trackGroupState.name}`), mithril('.fold-button', {
	    ...
	    onupdate({ dom }) {
	        ...
	        this.backgroundColor = getComputedStyle(dom)
	            .getPropertyValue("--collapsed-background");
	    }
	    renderCanvas(ctx, size) {
	        ctx.save();
