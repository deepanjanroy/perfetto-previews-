var perfetto = (function () {
	...
	***************************************************************************** */
	...
	var reducer_1 = reducer.rootReducer;
	...
	class Globals {
	    ...
	    runControllers() {
	        ...
	        for (let iter = 0; runAgain || this._queuedActions.length > 0; iter++) {
	            ...
	            for (const action of actions) {
	                ...
	                this._state = reducer.rootReducer(this.state, action);
	            }
	    ...
	    createEngine() {
	        return tslib_es6.__awaiter(this, void 0, void 0, function* () {
	            const portAndId = yield logging.assertExists(this._frontend)
	                .send('createEngine', []);
	            return wasm_engine_proxy.WasmEngineProxy.create(portAndId);
	        });
	    }
	    destroyEngine(id) {
	        return tslib_es6.__awaiter(this, void 0, void 0, function* () {
	            yield logging.assertExists(this._frontend).send('destroyEngine', [id]);
	        });
	    }
	...
	class TrackController extends controller.Controller {
	    ...
	    run() {
	        ...
	        if (dataReq === undefined)
	            return;
	        globals.globals.dispatch(actions.clearTrackDataRequest(this.trackId));
	...
	class CpuSliceTrackController extends track_controller.TrackController {
	    ...
	    onBoundsChange(start, end, resolution) {
	        ...
	        this.engine.rawQuery({ 'sqlQuery': query }).then(rawResult => {
	            ...
	            globals.globals.publish('TrackData', { id: this.trackId, data: slices });
	        });
	    }
	}
	    ...
	    run() {
	        ...
	        if (globals.globals.state.permalink.hash === undefined) {
	            PermalinkController.createPermalink().then(hash => {
	                globals.globals.dispatch(actions.setPermalink(requestId, hash));
	            });
	            return;
	        }
	        ...
	        PermalinkController.loadState(globals.globals.state.permalink.hash).then(state => {
	            globals.globals.dispatch(actions.setState(state));
	...
	class QueryController extends controller.Controller {
	        ...
	        switch (this.state) {
	            case 'init':
	                ...
	                this.runQuery(config.query).then(result => {
	                    ...
	                    globals.globals.dispatch(actions.deleteQuery(this.args.queryId));
	                });
	    ...
	    runQuery(sqlQuery) {
	        return tslib_es6.__awaiter(this, void 0, void 0, function* () {
	            ...
	            const rawResult = yield this.args.engine.rawQuery({ sqlQuery });
	...
	class TraceController extends controller.Controller {
	    ...
	    run() {
	        ...
	        switch (this.state) {
	            case 'init':
	                globals.globals.dispatch(actions.setEngineReady(this.engineId, false));
	                this.loadTrace().then(() => {
	                    globals.globals.dispatch(actions.setEngineReady(this.engineId, true));
	                });
	                globals.globals.dispatch(actions.updateStatus('Opening trace'));
	    ...
	    loadTrace() {
	        return tslib_es6.__awaiter(this, void 0, void 0, function* () {
	            globals.globals.dispatch(actions.updateStatus('Creating trace processor'));
	            const engineCfg = logging.assertExists(globals.globals.state.engines[this.engineId]);
	            this.engine = yield globals.globals.createEngine();
	            ...
	            if (engineCfg.source instanceof File) {
	                ...
	                for (let off = 0; off < blob.size; off += SLICE_SIZE) {
	                    ...
	                    globals.globals.dispatch(actions.updateStatus(`${statusHeader} ${progress} %`));
	                }
	            }
	            else {
	                const resp = yield fetch(engineCfg.source);
	                if (resp.status !== 200) {
	                    globals.globals.dispatch(actions.updateStatus(`HTTP error ${resp.status}`));
	                ...
	                for (let off = 0;;) {
	                    ...
	                    if (nowMs - tLastUpdateMs > 100) {
	                        ...
	                        globals.globals.dispatch(actions.updateStatus(status));
	                    }
	                    if (readRes.done)
	                        break;
	                }
	            ...
	            const actions$$1 = [
	                actions.setTraceTime(traceTime),
	                actions.navigate('/viewer'),
	            ];
	            if (globals.globals.state.visibleTraceTime.lastUpdate === 0) {
	                actions$$1.push(actions.setVisibleTraceTime(traceTime));
	            }
	    ...
	    listTracks() {
	        return tslib_es6.__awaiter(this, void 0, void 0, function* () {
	            globals.globals.dispatch(actions.updateStatus('Loading tracks'));
	            const engine = logging.assertExists(this.engine);
	            const addToTrackActions = [];
	            const numCpus = yield engine.getNumberOfCpus();
	            for (let cpu = 0; cpu < numCpus; cpu++) {
	                addToTrackActions.push(actions.addTrack(this.engineId, common$2.CPU_SLICE_TRACK_KIND, cpu));
	            }
	            const threadQuery = yield engine.rawQuery({
	            ...
	            for (let i = 0; i < threadQuery.numRecords; i++) {
	                ...
	                addToTrackActions.push(actions.addChromeSliceTrack(this.engineId, common$4.SLICE_TRACK_KIND, upid, utid, threadName, maxDepth));
	            }
	            globals.globals.dispatchMultiple(addToTrackActions);
	        });
	    }
	    listThreads() {
	        return tslib_es6.__awaiter(this, void 0, void 0, function* () {
	            globals.globals.dispatch(actions.updateStatus('Reading thread list'));
	            const sqlQuery = 'select utid, tid, pid, thread.name, process.name ' +
	                'from thread inner join process using(upid)';
	            const threadRows = yield logging.assertExists(this.engine).rawQuery({ sqlQuery });
	            const threads = [];
	    ...
	    loadTimelineOverview(traceTime) {
	        return tslib_es6.__awaiter(this, void 0, void 0, function* () {
	            ...
	            for (let step = 0; step < numSteps; step++) {
	                globals.globals.dispatch(actions.updateStatus('Loading overview ' +
	                ...
	                // Slices overview.
	                const slicesRows = yield engine.rawQuery({
	                    sqlQuery: `select sum(dur)/${stepSec}/1e9, process.name, process.pid, upid ` +
	...
	class AppController extends controller.Controller {
	    ...
	    run() {
	        const engineKeys = Object.keys(globals.globals.state.engines);
	        const childControllers = [
	            controller.Child('permalink', permalink_controller.PermalinkController, {}),
	        ];
	        if (engineKeys.length > 0) {
	            const cfg = globals.globals.state.engines[engineKeys[0]];
	            childControllers.push(controller.Child(cfg.id, trace_controller.TraceController, cfg.id));
	        }
	        return childControllers;
	    }
	}
