var perfetto = (function () {
	...
	exports.fnv1Hash = fnv1Hash;

	});

	unwrapExports(hash);
	var hash_1 = hash.fnv1Hash;
...
limit 20;`;
	...
	***************************************************************************** */
	...
	const MARGIN_TOP = 5.5;
	const RECT_HEIGHT = 30;
	// TODO(dproy): Reduce duplication with cpu slice frontend.
	function getCurResolution() {
	    // Truncate the resolution to the closest power of 10.
	    const resolution = globals.globals.frontendLocalState.timeScale.deltaPxToDuration(1);
	    return Math.pow(10, Math.floor(Math.log10(resolution)));
	...
	class TrackGroupPanel extends panel.Panel {
	    constructor({ attrs }) {
	        ...
	        this.hue = (128 + (32 * this.config.pid)) % 256;
	    }
	    get config() {
	    ...
	    view({ attrs }) {
	        const trackGroup = logging.assertExists(globals.globals.state.trackGroups[attrs.trackGroupId]);
	        // TODO: Move this to somewhere more generic.
	        const config = trackGroup.config;
	        return mithril('.process-panel', mithril('.shell', mithril('h1', trackGroup.name + config.pid), mithril('.fold-button', {
	            onclick: () => globals.globals.dispatch(actions.Actions.toggleTrackGroupCollapsed({
	                trackGroupId: attrs.trackGroupId,
	            })),
	        }, mithril('i.material-icons', trackGroup.collapsed ? 'expand_more' : 'expand_less'))));
	    }
	    oncreate(vnode) {
	        this.onupdate(vnode);
	    }
	    ...
	    reqDataDeferred() {
	        ...
	        const reqRes = getCurResolution();
	        this.reqPending = false;
	        globals.globals.dispatch(actions.Actions.reqTrackGroupData({
	            trackGroupId: this.trackGroupState.id,
	            start: reqStart,
	            end: reqEnd,
	            resolution: reqRes
	        }));
	    ...
	    data() {
	        return globals.globals.trackDataStore.get(this.trackGroupId);
	    }
	    renderCanvas(ctx, size) {
	        ...
	        const inRange = data !== undefined &&
	            (visibleWindowTime.start >= data.start &&
	                visibleWindowTime.end <= data.end);
	        if (!inRange || data && data.resolution !== getCurResolution()) {
	            // TODO(dproy): There should be a way to cancel data requests.
	            if (!this.reqPending) {
	                this.reqPending = true;
	                setTimeout(() => this.reqDataDeferred(), 50);
	            }
	...
	class TraceViewer {
	    ...
	    view() {
	        ...
	        // scrollingPanels.unshift(m(TrackGroupPanel, {key: 'process-panel'}));
