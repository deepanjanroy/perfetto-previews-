var perfetto = (function () {
	...
	unwrapExports(state);
	var state_1 = state.defaultTraceTime;
	var state_2 = state.createEmptyState;
	...
	***************************************************************************** */
	...
	class CpuCounterTrackController extends track_controller.TrackController {
	    onBoundsChange() { }
	}
	CpuCounterTrackController.kind = common.CPU_COUNTER_TRACK_KIND;
	track_controller.trackControllerRegistry.register(CpuCounterTrackController);
	...
	CpuSliceTrackController.kind = common$2.CPU_SLICE_TRACK_KIND;
	...
	ChromeSliceTrackController.kind = common$4.SLICE_TRACK_KIND;
	...
	var track_group_common = createCommonjsModule(function (module, exports) {
	// Copyright (C) 2018 The Android Open Source Project
	//
	// Licensed under the Apache License, Version 2.0 (the "License");
	// you may not use this file except in compliance with the License.
	...
	function assertConfig(config) {
	    ...
	    logging.assertTrue(typeof _config.upid === 'number' &&
	        typeof _config.pid === 'number');
	    return _config;
	}
	...
	unwrapExports(track_group_common);
	var track_group_common_1 = track_group_common.assertConfig;

	var track_group_controller = createCommonjsModule(function (module, exports) {
	// Copyright (C) 2018 The Android Open Source Project
	//
	// Licensed under the Apache License, Version 2.0 (the "License");
	// you may not use this file except in compliance with the License.
	...
	function isSequential(numArray) {
	    if (numArray.length === 0)
	        return true;
	    const copiedArray = [...numArray];
	    copiedArray.sort((a, b) => (b - a));
	    for (let i = 0; i < copiedArray.length - 1; i++) {
	        if (copiedArray[i] !== copiedArray[i] + i) {
	            return false;
	        }
	    }
	    return true;
	}
	function intersectDuration(a, b) {
	...
	class TrackGroupController extends controller.Controller {
	    constructor(args) {
	        super('main');
	        this.busy = false;
	        this.trackGroupId = args.trackGroupId;
	        this.engine = args.engine;
	    }
	    // TODO(dproy): This should be factored out.
	    query(query) {
	        return tslib_es6.__awaiter(this, void 0, void 0, function* () {
	            const result = yield this.engine.query(query);
	            if (result.error) {
	    ...
	    get config() {
	        return track_group_common.assertConfig(this.trackGroupState.config);
	    }
	    get trackGroupState() {
	        return logging.assertExists(globals.globals.state.trackGroups[this.trackGroupId]);
	    }
	    // TODO(dproy): Reduce duplication with track code.
	    update(start, end, resolution) {
	        return tslib_es6.__awaiter(this, void 0, void 0, function* () {
	            if (this.busy)
	                return;
	            ...
	            let sliceQueryStr;
	            if (isSequential(utids)) {
	                sliceQueryStr = `select ts, dur from slices where ts > ${startNs} and ts < ${endNs} ` +
	                    `and utid >= ${Math.min(...utids)} and utid <= ${Math.max(...utids)}`;
	            }
	            else {
	                sliceQueryStr = `select ts, dur from slices where ts > ${startNs} and ts < ${endNs} ` +
	                    `and utid in (${utids.join(',')})`;
	            }
	            const sliceQuery = yield this.query(sliceQueryStr);
	            const bucketSizeNs = Math.round(resolution * 10 * 1e9);
	            const numBuckets = Math.ceil((endNs - startNs) / bucketSizeNs);
	            const utilizations = new Float64Array(numBuckets);
	            // TODO: Write unit test for this.
	            for (let i = 0; i < sliceQuery.numRecords; i++) {
	                const sliceStart = sliceQuery.columns[0].longValues[0];
	                const sliceEnd = sliceStart +
	                    sliceQuery.columns[1].longValues[0];
	                const bucketStartIndex = Math.floor((sliceStart - startNs) / bucketSizeNs);
	                const bucketEndIndex = Math.floor((sliceEnd - startNs) / bucketSizeNs);
	                for (let b = bucketStartIndex; b <= bucketEndIndex; b++) {
	                    const bucketStart = startNs + b * bucketSizeNs;
	                    const bucketEnd = startNs + (b + 1) * bucketSizeNs;
	                    utilizations[b] += intersectDuration({ start: bucketStart, end: bucketEnd }, { start: sliceStart, end: sliceEnd });
	                }
	            }
	            globals.globals.publish('TrackData', {
	                id: this.trackGroupId,
	                data: { start, end, resolution, bucketSizeSeconds: 1, utilizations },
	            });
	            globals.globals.dispatch(actions.Actions.clearTrackGroupDataReq({ trackGroupId: this.trackGroupId }));
	            this.busy = false;
	        });
	    }
	    run() {
	        const dataReq = this.trackGroupState.dataReq;
	        if (dataReq) {
	            this.update(dataReq.start, dataReq.end, dataReq.resolution);
	        }
	        // globals.publish('TrackData', {id: this.trackGroupId, data: {}});
	    }
	}
	exports.TrackGroupController = TrackGroupController;

	});

	unwrapExports(track_group_controller);
	var track_group_controller_1 = track_group_controller.TrackGroupController;
	    ...
	    run() {
	        ...
	        switch (this.state) {
	            ...
	            case 'ready':
	                ...
	                for (const [trackGroupId, trackGroupCfg] of Object.entries(globals.globals.state.trackGroups)) {
	                    if (trackGroupCfg.engineId !== this.engineId)
	                        continue;
	                    const args = { trackGroupId, engine };
	                    childControllers.push(controller.Child(trackGroupId, track_group_controller.TrackGroupController, args));
	                }
	        ...
	        return tslib_es6.__awaiter(this, void 0, void 0, function* () {
	            ...
	            for (let cpu = 0; cpu < numCpus; cpu++) {
	                addToTrackActions.push(actions.Actions.addTrack({
	                    engineId: this.engineId,
	                    kind: common$2.CPU_SLICE_TRACK_KIND,
	                    name: `Cpu ${cpu}`,
	                    config: {
	                        cpu,
	                    }
	            ...
	            // Local experiments showed getting maxDepth separately is ~2x faster than
	            ...
	            const threadQuery = yield engine.query('select utid, tid, upid, pid, thread.name, process.name from thread inner join process using(upid)');
	            ...
	            for (let i = 0; i < threadQuery.numRecords; i++) {
	                ...
	                addToTrackActions.push(actions.Actions.addTrack({
	                    engineId: this.engineId,
	                    kind: common$4.SLICE_TRACK_KIND,
	                    name: threadName + `[${tid}]`,
	                    trackGroup: pUuid,
	                    config: { upid, utid, maxDepth },
	                }));
	            }
	            globals.globals.dispatchMultiple(addTrackGroupActions.concat(addToTrackActions));
	        });
	    }
	    listThreads() {
	        return tslib_es6.__awaiter(this, void 0, void 0, function* () {
	...
	var index = unwrapExports(controller$8);
